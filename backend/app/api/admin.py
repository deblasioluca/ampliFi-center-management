...
def _run_load_in_background(batch_id: int) -> None:
    ...
    try:
        ...
        clear_cancel(batch_id)
        # Clean up the uploaded file if load succeeded (status=loaded)
        try:
            batch = db.get(UploadBatch, batch_id)
            if batch and batch.status == "loaded" and batch.storage_uri:
            ...
            _log.warning("Could not delete upload file for batch %s: %s", batch_id, cleanup_err)
        db.close()
...
UPLOAD_TEMPLATES: dict[str, dict] = {
    ...
    "gl_accounts_group": {
        "filename": "template_gl_accounts_group.csv",
        "description": "GL Account upload — Group Reporting Chart of Accounts",
        "columns": [
            "MANDT",
            "KTOPL",
            "SAKNR",
